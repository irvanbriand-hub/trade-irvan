  ...
  return (
    <button
      ...
      className={`flex flex-col items-stretch gap-1 rounded-md border p-1.5 text-left transition-colors min-h-[64px] sm:min-h-[88px] ${
        inMonth ? "border-border bg-card hover:bg-accent" : "border-transparent bg-muted/20 text-muted-foreground"
      }`}
    >
      <span className={`text-xs font-medium tabular-nums ${today ? "text-primary font-bold" : ""}`}>
        {date.getDate()}
      </span>
        ...
        {visible.map((s) => {
          ...
          return (
            ...
            >
              ...
              <span className="truncate">{meta.emoji} {s.title}</span>
            </span>
          );
        })}
