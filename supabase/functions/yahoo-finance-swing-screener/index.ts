...
function evalKetat(
  ...
  return {
    vok, vokTipe, ketat, bullish, msp: msp_cond,
    ii, iiy, is_val, k5, d5, adx13, adxama,
    ma3, ma5, ma10, ma20, ma50,
    dma3, dma5, dma10, dma20, dma50, tma20, tma50,
    rp, rpP, vv1, vma60, v3ma60, v5ma60, v10ma60, v30ma90,
    macdKondisi, stochKondisi, adxKondisi,
  };
}

async function processStock(ticker: string, screenerType: string): Promise<any | null> {
  try {
    ...
    if (closes.length < 450) return null; // need 400+ for MA400
    ...
    const kResult = evalKetat(closes, highs, lows, volumes, macdArr, signalArr, kArr, dArr, adxArr, adxEma, i);
    if (!kResult) return null;

    // Extended MAs
    const ma100 = smaAt(closes, 100, i);
    const ma200 = smaAt(closes, 200, i);
    const ma400 = smaAt(closes, 400, i);
    ...
    // ABOVEALLMA
    const aboveAllMA = C >= kResult.ma3 && C >= kResult.ma5 && C >= kResult.ma10
      && C >= kResult.ma20 && (!isNaN(kResult.ma50) && C >= kResult.ma50)
      && (!isNaN(ma100) && C >= ma100) && (!isNaN(ma200) && C >= ma200)
      && (!isNaN(ma400) && C >= ma400) && (!isNaN(bbBottom) && C >= bbBottom);

    const maAboveCount = [kResult.ma3, kResult.ma5, kResult.ma10, kResult.ma20, kResult.ma50, ma100, ma200, ma400]
      .filter(m => !isNaN(m) && C >= m).length;

    // Previous day KETAT
    const kResultPrev = i >= 2 ? evalKetat(closes, highs, lows, volumes, macdArr, signalArr, kArr, dArr, adxArr, adxEma, i - 1) : null;
    const ketatYesterday = kResultPrev?.ketat || false;
    ...
    if (screenerType === "ketat_allma") {
      passes = kResult.ketat && aboveAllMA;
    } else if (screenerType === "ketat_pertama") {
      passes = kResult.ketat && !ketatYesterday;
    } else if (screenerType === "bottom_fishing") {
      passes = kResult.vok && kResult.is_val >= 0 && C > L && (
        (!isNaN(dma100) && dma100 < 0 && dma100 > -2) ||
        (!isNaN(clrbbb) && clrbbb < 0 && clrbbb > -2) ||
        (!isNaN(dma200) && dma200 < 0 && dma200 > -2) ||
        (!isNaN(kResult.dma50) && kResult.dma50 < 0 && kResult.dma50 > -2) ||
        (!isNaN(kResult.dma20) && kResult.dma20 < 0 && kResult.dma20 > -2) ||
    ...
    // Find nearest MA for bottom fishing
    let nearestMA = "";
    let nearestMADist = 999;
    if (screenerType === "bottom_fishing") {
      const candidates = [
        { name: "MA20", dist: kResult.dma20 },
        { name: "MA50", dist: kResult.dma50 },
      ...
      if (candidates.length > 0) {
        ...
        nearestMA = candidates[0].name;
        nearestMADist = candidates[0].dist;
      }
    }
    ...
    return {
      ...
      vv1: kResult.vv1, vma60: kResult.vma60, v3ma60: kResult.v3ma60,
      v5ma60: kResult.v5ma60, v10ma60: kResult.v10ma60, v30ma90: kResult.v30ma90,
      vok: kResult.vok, vokTipe: kResult.vokTipe, rp: kResult.rp,
      ma3: kResult.ma3, ma5: kResult.ma5, ma10: kResult.ma10,
      ma20: kResult.ma20, ma50: isNaN(kResult.ma50) ? 0 : kResult.ma50,
      ma100: isNaN(ma100) ? 0 : ma100, ma200: isNaN(ma200) ? 0 : ma200,
      ma400: isNaN(ma400) ? 0 : ma400, bbBottom: isNaN(bbBottom) ? 0 : bbBottom,
      dma3: kResult.dma3, dma5: kResult.dma5, dma10: kResult.dma10,
      dma20: kResult.dma20, dma50: kResult.dma50,
      dma100: isNaN(dma100) ? 0 : dma100, dma200: isNaN(dma200) ? 0 : dma200,
      dma400: isNaN(dma400) ? 0 : dma400, clrbbb: isNaN(clrbbb) ? 0 : clrbbb,
      tma20: kResult.tma20, tma50: kResult.tma50,
      ii: kResult.ii, iiy: kResult.iiy, is_val: kResult.is_val,
      k5: isNaN(kResult.k5) ? 0 : kResult.k5, d5: isNaN(kResult.d5) ? 0 : kResult.d5,
      adx13: isNaN(kResult.adx13) ? 0 : kResult.adx13,
      ketat: kResult.ketat, bullish: kResult.bullish, msp: kResult.msp,
      aboveAllMA, maAboveCount, ketatYesterday,
      nearestMA, nearestMADist,
      macdKondisi: kResult.macdKondisi, stochKondisi: kResult.stochKondisi, adxKondisi: kResult.adxKondisi,
    };
  } catch {
    return null;
  }
...
serve(async (req) => {
  ...
  try {
    let batchIndex = 0, batchSize = 100, screenerType = "ketat_allma";
    try {
      const b = await req.json();
      batchIndex = b?.batchIndex ?? 0;
      if (b?.batchSize) batchSize = b.batchSize;
      if (b?.screenerType) screenerType = b.screenerType;
    } catch {}
    ...
    const results: any[] = [];
    for (let j = 0; j < tickers.length; j += 20) {
      const batch = tickers.slice(j, j + 20);
      const batchRes = await Promise.all(batch.map(t => processStock(t, screenerType)));
      for (const r of batchRes) { if (r) results.push(r); }
    }

    return new Response(JSON.stringify({
      stocks: results, totalBatches, batchIndex,
      totalTickers: IDX_TICKERS.length, processedInBatch: tickers.length, screenerType,
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
